...
class AStar:
    ...
    def generate_target_course(self):
        ...
        print(len(self.initial_path), len(self.smoothed_path))
        for i in range(len(self.initial_path)):
            if not self.initial_path[i].is_junction:
                dist = [math.hypot(pn.x - self.initial_path[i].x, pn.y - self.initial_path[i].y) for pn in self.smoothed_path]
                index = dist.index(min(dist))
                self.smoothed_path[index] = self.initial_path[i]
        if self.smoothed_path is not None and False:
            for p in self.smoothed_path:
                ...
                self.world.debug.draw_string(carla.Location(x=p.x, y=p.y), "+", draw_shadow=False,
                                        ...
                                        life_time=30.5, persistent_lines=True)
